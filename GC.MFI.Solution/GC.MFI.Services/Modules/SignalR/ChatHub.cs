...
public class ChatHub : Hub
{
    ...
    public ChatHub(ISignalRConnectionTableService service, IHttpContextAccessor httpContextAccessor, INotificationTableService notificationTableService)
    {
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        if (token != null)
        {
            ...
            var getNotification = notificationTableService.GetMany(t => t.Push == false && t.ReceiverID == memberId).ToList().OrderByDescending(t => t.UpdateDate);

            await Clients.Client(connectionId).SendAsync("OLD", getNotification);
            var getnewNotification = notificationTableService.GetMany(t => t.Push == true && t.ReceiverID == memberId).ToList().OrderByDescending(t => t.UpdateDate);
